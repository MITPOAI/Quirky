...
## Diagnose layer — see *why*, fix *only what's wrong*, prove it *honestly*

Beyond a single score, Quirky now explains and prescribes. Everything below is CPU-only classical signal-processing (no weights, no network) and is exposed through the CLI, the web dashboard, the API, and the MCP tools.

**Slop X-ray — explainable heatmap.** Instead of "this is 0.29 AI", render a heatmap that shows *where* an image reads as synthetic (too-clean skin, flat lighting, suspicious symmetry, grid residue), weighted toward the actual subject.

```bash
uv run quirky xray -a ai.png -o xray.png     # writes a heatmap overlay + prints hotspots
```

**Prescriptive fix cards.** `diagnose` measures the specific, named defects an image has and emits one card per defect — severity, a plain-English detail, and the exact fix it recommends. In the dashboard these become accept/reject cards; the ids map 1:1 onto the humanizer, so your selections drive the edit.

```bash
uv run quirky diagnose -a ai.png             # JSON: defects[], recommended_fixes[]
uv run quirky humanize -a ai.png -o out.png --fixes white_balance,clahe_lighting,plastic_texture
```

**Generator fingerprinting.** A weight-free heuristic guess of which generator likely made an image (SD/SDXL VAE 8px grid, GAN checkerboard, over-smoothed render, generic diffusion, or *real camera*), plus the inverse corrections that signature calls for. A statistical prior, not a trained classifier.

```bash
uv run quirky fingerprint -a ai.png          # ranked sources + inverse_fixes
```

**Minimal-edit fidelity lock.** Least intervention: climb intensity only until the target is met while structural similarity (SSIM) to the original stays above a floor — so the image is never over-cooked.

```bash
uv run quirky humanize -a ai.png -o out.png --lock --target 0.15 --min-ssim 0.86
```

**Honesty audit — don't grade your own homework.** Score before/after against an *external* oracle that is separate from the metric Quirky optimizes. Ships a weight-free ensemble oracle; drop in a real ONNX detector via `quirky[dl]` (`QUIRKY_ORACLE_MODEL`) for publishable claims.

```bash
uv run quirky audit ai.png out.png           # external P(AI): before -> after, true reduction
```

New MCP tools: `quirky_diagnose_image`, `quirky_xray_image`, `quirky_fingerprint_image`, `quirky_humanize_locked`, `quirky_audit_transfer`.

---

## Clean + Re-map — scrub what's embedded, then look again

Two more CPU-only, weight-free stages that close the loop the Diagnose layer opened: **Clean** answers "what's actually left in this file", and **Re-map** answers "did that edit actually work, and did it break anything new".

**Clean — metadata scrub.** The detector already flags generator-metadata leaks (`prompt_leak_score`); Clean is the other half — report every embedded field in plain language, then strip it via a clean re-encode (Pillow only writes EXIF/PNG-text when you explicitly pass it back in, so a plain re-save drops it all). Structural codec fields (JFIF version, DPI, gamma — regenerated by any encoder) are never counted as "not clean"; only genuine EXIF / PNG-text / generator-parameter leaks are. Optionally leave a single honest `"Powered by Quirky (MITPO)"` tag instead of a silent blank.

```bash
uv run quirky clean -a ai.png --scan-only               # report only, writes nothing
uv run quirky clean -a ai.png -o clean.png               # strip everything embedded
uv run quirky clean -a ai.png -o clean.png --attribute   # strip, then leave one honest tag
```

**Re-map — "map it again".** After any edit, re-run the same diagnosis on the output that ran on the input, and report what actually changed: which defects resolved, which remain, and — the important one — whether the edit introduced anything *new* that wasn't flagged before (the signature of over-cooking an image), plus a red/green delta heatmap. `remap-loop` chains diagnose → humanize → re-map across multiple rounds and stops on convergence, a fidelity-floor breach vs. the original, or a round cap.

```bash
uv run quirky remap before.png after.png -o delta.png              # resolved/remaining/new + delta heatmap
uv run quirky remap-loop -a ai.png -o final.png --min-ssim 0.80 --max-rounds 3
```

New MCP tools: `quirky_scan_metadata`, `quirky_clean_metadata`, `quirky_remap_image`, `quirky_remap_loop`.
